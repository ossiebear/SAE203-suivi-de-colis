...
function testerTables($connex) {
    try {
        $sqlClients = "SELECT COUNT(*) as nb_clients FROM clients";
        $stmtClients = $connex->prepare($sqlClients);
        $stmtClients->execute();
        $nbClients = $stmtClients->fetchColumn();

        $sqlPackages = "SELECT COUNT(*) as nb_packages FROM packages";
        $stmtPackages = $connex->prepare($sqlPackages);
        $stmtPackages->execute();
        $nbPackages = $stmtPackages->fetchColumn();

        echo "<!-- DEBUG: $nbClients clients et $nbPackages colis dans la base -->";

        return ['clients' => $nbClients, 'packages' => $nbPackages];
    } catch (Exception $e) {
